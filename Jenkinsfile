...
node {
    ...
    // Hardcoded scratch org usernames
    def SCRATCH_ORG_1_USERNAME = 'test-n64neb1z2clc@example.com'
    def SCRATCH_ORG_2_USERNAME = 'test-jq3rrrmpq7zc@example.com'
    ...
    withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_file')]) {
        ...
        stage("Deploy to ScratchOrg1") {
            def result = bat(returnStdout: true, script: "${sfcli} project deploy start --manifest manifest/package.xml --target-org ${SCRATCH_ORG_1_USERNAME}")
            println result
        }

        stage("Deploy to ScratchOrg2") {
            def result = bat(returnStdout: true, script: "${sfcli} project deploy start --manifest manifest/package.xml --target-org ${SCRATCH_ORG_2_USERNAME}")
            println result
        }
    }
}
